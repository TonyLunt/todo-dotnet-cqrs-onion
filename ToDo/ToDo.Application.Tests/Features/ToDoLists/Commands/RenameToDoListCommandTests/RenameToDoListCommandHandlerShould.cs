...
{
    public class RenameToDoListCommandHandlerShould
    {
        [Fact]
        public async Task ReturnViewModelIfValid()
        {
            ...
            var handler = new RenameToDoListCommandHandler(mockRepository.Object, new Mock<ILogger<RenameToDoListCommandHandler>>().Object);

            var response = await handler.Handle(new RenameToDoListCommand(), new System.Threading.CancellationToken());
        ...
        public async Task ThrowAsyncIfNotExists()
        {
            ...
            await Assert.ThrowsAsync<NotFoundException>(async() => await handler.Handle(new RenameToDoListCommand(), new System.Threading.CancellationToken()));
